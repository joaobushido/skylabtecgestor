/* =========================================================
   ...
   Firebase-integrated: Auth, Firestore, real-time listeners
   ========================================================= */

import {
  auth, db,
  requireAuth, ensureUserDoc,
...
// ── Search Cache ──────────────────────────────────────────
let searchCache = { services: [], warranties: [], inventory: [], clients: [] };
...
function timeAgo(ts) {
  if (!ts) return '';
  const date = ts instanceof Timestamp ? ts.toMillis() : ts;
...
function switchNotifTab(tab) {
  activeNotifTab = tab;
  $$('.notif-panel__tab').forEach(t => t.classList.toggle('is-active', t.dataset.tab === tab));
  renderNotifications();
}
...
function renderNotifications() {
  ...
  const tabSys = $('#tab-sistema-count');
  const tabAdm = $('#tab-admin-count');
  if (tabSys) tabSys.textContent = unreadSystem;
  if (tabAdm) tabAdm.textContent = unreadAdmin;
...
function startNotificationListeners(uid) {
  // Admin notifications from global collection
  try {
    onSnapshot(
      query(collection(db, 'announcements'), orderBy('createdAt', 'desc'), limit(20)),
      (snap) => {
        adminNotifications = snap.docs.map(d => ({
          id: d.id,
          type: d.data().type || 'info',
          title: d.data().title || 'Aviso',
  ...
  } catch (e) { console.warn('Announcements listener setup failed:', e); }
  ...
  try {
    onSnapshot(collection(db, 'users', uid, 'inventory'), (snap) => {
      ...
      snap.forEach(d => {
        ...
        if (qty <= 2) {
          stockNotifs.push({
            ...
            type: 'stock',
            title: 'Estoque Baixo',
            message: `${data.name || 'Item'} — apenas ${qty} unidade(s) restante(s).`,
      ...
      systemNotifications = [
        ...systemNotifications.filter(n => !n.id.startsWith('stock-')),
        ...stockNotifs
      ];
      renderNotifications();
    }, (err) => console.warn('Inventory listener error:', err));
  } catch (e) { console.warn('Inventory listener setup failed:', e); }

  // Warranty expiry alerts
  try {
    onSnapshot(collection(db, 'users', uid, 'warranties'), (snap) => {
      const warNotifs = [];
      const now = Date.now();
      snap.forEach(d => {
        const data = d.data();
        if (data.endDate) {
          const end = typeof data.endDate === 'string' ? new Date(data.endDate).getTime() :
                      data.endDate instanceof Timestamp ? data.endDate.toMillis() : data.endDate;
          ...
          if (daysLeft > 0 && daysLeft <= 7) {
            warNotifs.push({
              ...
              message: `${data.client || 'Cliente'} — ${data.device || 'Dispositivo'}: ${daysLeft} dia(s) restante(s).`,
              timestamp: data.createdAt || Date.now(),
              link: 'pages/warranty.html'
            });
          }
        }
      });
      systemNotifications = [
        ...systemNotifications.filter(n => !n.id.startsWith('warranty-')),
        ...warNotifs
      ];
      renderNotifications();
    }, (err) => console.warn('Warranties listener error:', err));
  } catch (e) { console.warn('Warranties listener setup failed:', e); }

  // New services listener
  try {
    onSnapshot(
      query(collection(db, 'users', uid, 'services'), orderBy('createdAt', 'desc'), limit(5)),
      (snap) => {
        const svcNotifs = [];
          ...
          if (change.type === 'added') {
            ...
            if (data.createdAt) {
              ...
              if (Date.now() - ts < 3600000) {
                svcNotifs.push({
                  id: 'svc-' + change.doc.id,
                  type: 'success',
                  title: 'Novo Serviço',
                  message: `${data.client || 'Cliente'} - ${data.model || 'Dispositivo'}: ${data.serviceType || 'Reparo'}`,
                  timestamp: data.createdAt,
                  link: 'pages/technical-control.html'
                });
              }
            }
          }
        ...
        if (svcNotifs.length) {
          systemNotifications = [...svcNotifs, ...systemNotifications.filter(n => !svcNotifs.some(s => s.id === n.id))];
          renderNotifications();
        }
      },
      (err) => console.warn('Services listener error:', err)
    );
  } catch (e) { console.warn('Services listener setup failed:', e); }
}

// ══════════════════════════════════════════════════════════
// GLOBAL SEARCH — Firestore data
// ══════════════════════════════════════════════════════════
...
async function loadSearchData(uid) {
  try {
    const [svcSnap, warSnap, invSnap] = await Promise.all([
    ...
    searchCache.services = svcSnap.docs.map(d => ({ id: d.id, ...d.data() }));
    searchCache.warranties = warSnap.docs.map(d => ({ id: d.id, ...d.data() }));
    searchCache.inventory = invSnap.docs.map(d => ({ id: d.id, ...d.data() }));

    // Build clients from services
    const clientMap = {};
    searchCache.services.forEach(s => {
      if (s.client && !clientMap[s.client]) {
        clientMap[s.client] = { id: 'cl-' + Object.keys(clientMap).length, name: s.client, phone: s.phone || '', email: s.email || '' };
      }
    });
    searchCache.clients = Object.values(clientMap);
    ...
    html += invHits.map(i => `
      <div class="search-panel__item" onclick="window.location.href='pages/inventory.html?item=${i.id}'">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div class="search-panel__item-sub">${i.code ? 'Cód ' + highlight(i.code, t) + ' · ' : ''}Qtd ${i.quantity || 0}</div>
        </div>
        ${(parseInt(i.quantity) || 0) <= 0 ? '<span class="search-panel__item-badge" style="background:rgba(239,68,68,.12);color:var(--red)">Sem estoque</span>' : ''}
      </div>`).join('');
  }
  if (warHits.length) {
...
async function loadConfigFromFirestore(uid) {
  try {
    const snap = await getDoc(doc(db, 'users', uid, 'preferences', 'homeShortcuts'));
    if (snap.exists()) return sanitize(snap.data());
  } catch (e) { console.warn('Shortcuts load error:', e); }
  return clone(DEFAULT_CONFIG);
}

async function saveConfigToFirestore(uid, cfg) {
  try {
    await setDoc(doc(db, 'users', uid, 'preferences', 'homeShortcuts'), {
      ...cfg, updatedAt: serverTimestamp()
    });
  } catch (e) { console.warn('Shortcuts save error:', e); }
}
...
function editItemHTML(kind, idx, item) {
  ...
  if (isExpanded) {
    if (kind === 'buttons') {
      fields = `<div class="edit-item__body"><div class="edit-item__row">
        <div class="edit-item__field"><label>Texto</label><input data-field="label" value="${escHtml(item.label)}" maxlength="40" /></div>
        <div class="edit-item__field"><label>Estilo</label><select data-field="style">
          <option value="primary" ${item.style === 'primary' ? 'selected' : ''}>Primário</option>
      ...
      fields = `<div class="edit-item__body"><div class="edit-item__row">
        ...
        <div class="edit-item__field"><label>Cor</label><select data-field="color">${COLOR_OPTIONS.map(co => `<option value="${co}" ${color === co ? 'selected' : ''}>${co}</option>`).join('')}</select></div>
      </div><div class="edit-item__row edit-item__row--full">
  ...
  return `<div class="edit-item ${isExpanded ? 'is-expanded' : ''}" data-kind="${kind}" data-idx="${idx}" draggable="true">
    <div class="edit-item__head">
      ...
      <div class="edit-item__info"><div class="edit-item__name">${escHtml(title)}</div><div class="edit-item__sub">${escHtml(sub)}</div></div>
...
function bindEditorEvents() {
  $$('#home-edit-modal .edit-item').forEach(el => {
    ...
    el.addEventListener('dragstart', (e) => { el.style.opacity = '.4'; e.dataTransfer.setData('text/plain', key); e.dataTransfer.effectAllowed = 'move'; });
    el.addEventListener('dragend', () => el.style.opacity = '');
    el.addEventListener('dragover', (e) => { e.preventDefault(); el.style.borderColor = 'rgba(43,98,232,.5)'; });
...
function openPicker(kind) {
  ...
  if (draftConfig[kind].length >= max) { alert(`Limite máximo de ${max} ${kind === 'buttons' ? 'botões' : 'cards'} atingido.`); return; }
  pickerKind = kind;
  renderPicker('');
  $('#home-picker')?.classList.add('is-open');
  const s = $('#home-picker-search'); s.value = '';
  setTimeout(() => s?.focus(), 50);
}
function closePicker() { $('#home-picker')?.classList.remove('is-open'); }
...
function renderPicker(filter) {
  ...
  list.innerHTML = items.map((p, i) => `
    <div class="picker-item" data-pick="${i}">
      ...
      <div style="flex:1;min-width:0;"><div class="picker-item__label">${escHtml(p.label)}</div><div class="picker-item__url">${escHtml(p.url)}</div></div>
    </div>
  ...
  list.querySelectorAll('.picker-item').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (pickerKind === 'buttons') {
        draftConfig.buttons.push({ id: 'b' + Date.now(), label: page.label, icon: page.icon, url: page.url, style: draftConfig.buttons.length === 0 ? 'primary' : 'secondary' });
      } else {
        draftConfig.cards.push({ id: 'c' + Date.now(), title: page.label, desc: '', icon: page.icon, url: page.url, color: page.color || 'brand' });
      }
      closePicker();
      renderEditor(draftConfig);
    });
...
// Modal controls
function openEditModal() { draftConfig = clone(currentConfig); expandedKey = null; renderEditor(draftConfig); $('#home-edit-modal')?.classList.add('is-open'); document.body.style.overflow = 'hidden'; }
function closeEditModal() { $('#home-edit-modal')?.classList.remove('is-open'); document.body.style.overflow = ''; }
...
$('#home-edit-reset')?.addEventListener('click', () => { if (confirm('Restaurar os atalhos padrão?')) { draftConfig = clone(DEFAULT_CONFIG); renderEditor(draftConfig); } });

$('#home-edit-save')?.addEventListener('click', async () => {
  currentConfig = sanitize(draftConfig);
  renderShortcuts(currentConfig);
  localStorage.setItem(DISMISS_KEY, '1');
  $('#home-customize-banner')?.classList.add('hidden');
  closeEditModal();
  if (currentUser) await saveConfigToFirestore(currentUser.uid, currentConfig);
});
...
document.addEventListener('click', (e) => { if (fabOpen && !$('#fab-container')?.contains(e.target)) toggleFab(); });

// ══════════════════════════════════════════════════════════
// ANNOUNCEMENT MODAL — Firestore
// ══════════════════════════════════════════════════════════
...
function showAnnouncement(text, link, buttonText) {
  ...
  if (link) { linkEl.href = link; linkEl.classList.remove('hidden'); if (buttonText) btnTextEl.textContent = buttonText; }
  else { linkEl.classList.add('hidden'); }
  overlay.classList.add('is-open');
}

$('#announcement-close')?.addEventListener('click', () => $('#announcement-overlay')?.classList.remove('is-open'));
$('#announcement-dismiss')?.addEventListener('click', () => $('#announcement-overlay')?.classList.remove('is-open'));
$('#announcement-overlay')?.addEventListener('click', (e) => { if (e.target.id === 'announcement-overlay') $('#announcement-overlay').classList.remove('is-open'); });

async function checkAnnouncements() {
  try {
    const snap = await getDocs(query(collection(db, 'announcements'), where('active', '==', true), orderBy('createdAt', 'desc'), limit(1)));
    if (snap.empty) return;
    const data = snap.docs[0].data();
    const seenKey = 'skylab:ann:' + snap.docs[0].id;
    if (localStorage.getItem(seenKey)) return;
    localStorage.setItem(seenKey, '1');
    showAnnouncement(data.text || data.message || '', data.link || null, data.buttonText || 'Saiba Mais');
  } catch (e) { console.warn('Announcements check error:', e); }
}
...
// ══════════════════════════════════════════════════════════
// PRESENCE — Keep online status
// ══════════════════════════════════════════════════════════

function startPresence(uid) {
  const ref = doc(db, 'users', uid);
  const update = () => { updateDoc(ref, { isOnline: true, lastActive: serverTimestamp() }).catch(() => {}); };
  update();
  setInterval(update, 30000);
  window.addEventListener('beforeunload', () => {
    updateDoc(ref, { isOnline: false, lastActive: serverTimestamp() }).catch(() => {});
  });
}
...
async function initDashboard(user) {
  ...
  currentConfig = await loadConfigFromFirestore(user.uid);
  renderShortcuts(currentConfig);
  ...
  // Show customize banner
  const dismissed = localStorage.getItem(DISMISS_KEY) === '1';
  if (!dismissed) $('#home-customize-banner')?.classList.remove('hidden');
  ...
  try {
    ...
    if (settingsSnap.exists()) {
      ...
      if (data.workHoursOpen && data.workHoursClose) {
        const hoursEl = $('#footer-work-hours');
        if (hoursEl) { hoursEl.textContent = `Seg–Sex · ${data.workHoursOpen} – ${data.workHoursClose}`; hoursEl.classList.remove('hidden'); }
      }
    }
  } catch (e) { console.warn('Settings load error:', e); }

  // Load search data
  await loadSearchData(user.uid);

  // Start real-time listeners
  startNotificationListeners(user.uid);
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      if (data.accessExpiresAt) {
        const expiresMs = data.accessExpiresAt instanceof Timestamp ? data.accessExpiresAt.toMillis() : new Date(data.accessExpiresAt).getTime();
        const daysLeft = Math.ceil((expiresMs - Date.now()) / 86400000);
        if (daysLeft <= 7 && daysLeft > 0) {
          const banner = $('#plan-banner');
          const bannerText = $('#plan-banner-text');
          if (banner && bannerText) {
            bannerText.textContent = `Seu plano expira em ${daysLeft} dia(s). Considere renovar.`;
            banner.classList.remove('hidden');
            if (daysLeft <= 3) banner.classList.add('banner--danger');
            else banner.classList.add('banner--warning');
          }
        }
      }
    }
  } catch (e) { console.warn('Plan check error:', e); }
}
